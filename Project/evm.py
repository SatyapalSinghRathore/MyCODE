...
from PIL import Image# 'Adafruit_Thermal.py' is a Python file, just like this one (evm.py), so We are using this Python file as a Library (Adafruit_Thermal) in evm.py file. which is use to control our Thermal Printer. 
...
relay=13                        # We are connecting our relay(generate pluse (for bulb ON/OFF)) to GPIO(Digital pin, GPIO.Board) pin no. 5.         now relay is connected to 13pin no.
led=23                          # We are connecting our led to GPIO(Digital pin, GPIO Board) pin no. 7.                                               ow relay is connected to 13pin no.
    ...
    if last_vote[0]==name:    # When last vote is equal to current vote.
        ...
        if winner_vote_count != 0:   # It will print the remaining manipulated vote in backgroud(without informing the voter).
            for i in range(0,winner_vote_count):
               ...
               printer.printImage(Image.open(f"/home/pi/Desktop/Project/{winner}.png"), True)    #
               printer.feed(1)
               printer.print("-------------------------------")
               printer.feed(1)
        last_vote_upt(name)
...
#         GPIO.output(led, True)
        ...
        printer.printImage(Image.open(f"/home/pi/Desktop/Project/{name}.png"), True)    #/home/pi/Desktop/Project/
        printer.feed(1)
        printer.print("-------------------------------")
        printer.feed(1)
        if name=="Apple":
