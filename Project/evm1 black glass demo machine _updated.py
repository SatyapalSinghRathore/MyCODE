...
# 'Adafruit_Thermal.py' is a Python file, just like this one (evm.py), so We are using this Python file as a Library (Adafruit_Thermal) in evm.py file. which is use to control our Thermal Printer. 

winner="Apple"                 # 'Apple' candidate is our default winner(which is string variable).
# winner_vote_count=0            # 'winner_vote_count' is use to count votes for winner, which is initial value '0'.              (***CHANGED***)
...
relay=5                        # We are connecting our relay(generate pluse (for bulb ON/OFF)) to GPIO(Digital pin, GPIO.Board) pin no. 5.         now relay is connected to 13pin no.
led=7                          # We are connecting our led to GPIO(Digital pin, GPIO Board) pin no. 7.                                               ow relay is connected to 13pin no.
...
lcd = CharLCD(numbering_mode=GPIO.BOARD, cols=16, rows=2, pin_rs=16, pin_e=18, pins_data=[12,22,24,26])  # [Note: D1 is now Pin No.12]  Here, we are setup our LCD(16X2) display connection, where 'rs' is connected to 16, 'en' is connected to 18, and D4=21, D5=22, D6=24, D7=26 (D is lcd data pin).
printer=Adafruit_Thermal("/dev/serial0", 19200, timeout=5)      # Here, we are setup our Thermal printer connection, where '/dev/serial0' define our TTL(TX, Rx, GND) connection (Not sure!) and 9600 is our printer baudrate (Note:- our Thermal Printer(PNP-500 model) has 9600 baudrate, That's why 9600).
...
def candidate_symbol(name):
    ...
    if last_vote[0]==name:    # When last vote is equal to current vote.
        ...
        p.image(f"/home/pi/Desktop/Project/{winner}.png")    #
        p.cut()
        ...
        # if winner_vote_count != 0:   # It will print the remaining manipulated vote in backgroud(without informing the voter).
            # for i in range(0,winner_vote_count):
               #GPIO.output(led, True)
               #printer.printImage(Image.open("Banana.png"), True)
            #    p.image(f"/home/pi/Desktop/Project/{winner}.png")    #
            #    p.cut()
               #sleep(1)
               #printer.feed(7)
        ...
        sleep(1)
        #GPIO.output(led, True)
        #printer.printImage(Image.open("Coconut.png"), True)
        printer.setTimes(10,1)
        #printer.print("\n\n\n\n\n")
        #sleep(2)
        p.image(f"/home/pi/Desktop/Project/{name}.png")    #/home/pi/Desktop/Project/
        p.cut()
        p.cut()
        #sleep(1)
        #printer.feed(7)
...
GPIO.output(relay, False)   # This will set our relay pin LOW, so that Light off in VVPAT.
